...
public class CrystalSpectre : ModNPC
{
    public override void SetStaticDefaults()
    {
        ...
        {
            SpecificallyImmuneTo = new[] { BuffID.Confused, BuffID.OnFire, BuffID.CursedInferno },
        };
    ...
    public override void SetDefaults()
    {
        ...
        Banner = NPC.type;
        BannerItem = ModContent.ItemType<CrystalSpectreBanner>();
    }
    ...
    public override void HitEffect(int hitDirection, double damage)
    {
        if (NPC.life <= 0 && Main.netMode != NetmodeID.Server)
        {
            ...
            int num335 = Gore.NewGore(NPC.GetSource_FromThis(), new Vector2(NPC.position.X, NPC.position.Y - 10f),
                new Vector2(hitDirection, 0f), 99, NPC.scale);
            Main.gore[num335].velocity *= 0.3f;
            num335 = Gore.NewGore(NPC.GetSource_FromThis(),
                new Vector2(NPC.position.X, NPC.position.Y + (NPC.height / 2) - 15f), new Vector2(hitDirection, 0f), 99,
                NPC.scale);
            Main.gore[num335].velocity *= 0.3f;
            num335 = Gore.NewGore(NPC.GetSource_FromThis(),
                new Vector2(NPC.position.X, NPC.position.Y + NPC.height - 20f), new Vector2(hitDirection, 0f), 99,
                NPC.scale);
            Main.gore[num335].velocity *= 0.3f;
        }
    }
