...
public class AegisHallowor : ModNPC
{
    public override void SetStaticDefaults()
    {
        ...
        {
            SpecificallyImmuneTo = new[] { BuffID.Confused, ModContent.BuffType<Frozen>() },
        };
    ...
    public override void SetDefaults()
    {
        ...
        NPC.value = 6500;
        Banner = NPC.type;
        BannerItem = ModContent.ItemType<AegisHalloworBanner>();
    }
    ...
    public override void AI()
    {
        ...
        num73 = 4.2f;
        num74 = 0.022f;
        var vector11 = new Vector2(NPC.position.X + (NPC.width * 0.5f), NPC.position.Y + (NPC.height * 0.5f));
        ...
        num75 -= vector11.X;
        num76 -= vector11.Y;
        float num77 = (float)Math.Sqrt((num75 * num75) + (num76 * num76));
        float num78 = num77;
        if (num77 == 0f)
        {
        ...
        if (Main.rand.Next(20) == 0)
        {
            ...
            int num85 = Dust.NewDust(new Vector2(NPC.position.X, NPC.position.Y + (NPC.height * 0.25f)), NPC.width,
                (int)(NPC.height * 0.5f), DustID.Pixie, NPC.velocity.X, 2f, 75, NPC.color, NPC.scale);
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient && !Main.player[NPC.target].dead)
        {
            ...
            if (NPC.localAI[0] == 180f)
            {
                        ...
                        Main.player[NPC.target].width, Main.player[NPC.target].height))
                {
                    ...
                    int number2 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y,
                        -(float)Math.Cos(num1191) * 7f, -(float)Math.Sin(num1191) * 7f,
                        ModContent.ProjectileType<Projectiles.Hostile.HallowSpit>(), 70, 1f, NPC.target);
                    ...
                    float num1192 =
                        (float)Math.Atan2(vector158.Y - (player5.position.Y + (player5.height * 0.5f) - 40f),
                            vector158.X - (player5.position.X + (player5.width * 0.5f) - 40f));
                    int num1193 = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center.X, NPC.Center.Y,
                        -(float)Math.Cos(num1192), -(float)Math.Sin(num1192),
                        ModContent.ProjectileType<Projectiles.Hostile.HallowSpit>(), 70, 1f, NPC.target);
                    ...
                    NPC.NewNPC(NPC.GetSource_FromAI(), (int)(NPC.Center.X + NPC.velocity.X),
                        (int)(NPC.Center.Y + NPC.velocity.Y), ModContent.NPCType<HallowSpit>());
                }
    ...
    public override void HitEffect(int hitDirection, double damage)
    {
        if (NPC.life <= 0)
        {
            var R = new Rectangle((int)NPC.position.X, (int)(NPC.position.Y + ((NPC.height - NPC.width) / 2)),
                NPC.width, NPC.width);
            int C = 50;
            float vR = .4f;
            for (int i = 1; i <= C; i++)
            {
                int D = Dust.NewDust(NPC.position, R.Width, R.Height, DustID.Enchanted_Gold, 0, 0, 100, new Color(),
                    2f);
