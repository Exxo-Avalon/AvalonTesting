...
public class PhantasmBlasfah : ModNPC
{
        ...
        if(NPC.ai[0] == 3)
        {
            ...
            if(hasDashed)
            {
                ...
                if(currentPhase == 2)
                {
                    ...
                    if (swordDashTimer == 8)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<Projectiles.Hostile.SoulDagger>(), 35, default, Main.myPlayer, 0f, 7f);
                        swordDashTimer = 0;
                    }
                }
                NPC.velocity *= 0.98f;
    ...
    public void SpawnBlades(int amount)
    {
        ...
        {
            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<Projectiles.Hostile.SoulDagger>(), 35, 0, Main.myPlayer, 0f, i);
        }
    }
