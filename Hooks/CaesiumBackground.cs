...
public static class CaesiumBackground
{
    public static void ILCaesium()
    {
    ...
    private static void Main_DrawUnderworldBackgroudLayer(ILContext il)
    {
        ...
        c.EmitDelegate<Action<bool, Vector2, float, int>>((flat, screenOffset, pushUp, layerTextureIndex) =>
        {
            ...
            {
                assets[i] = ModContent.Request<Texture2D>("AvalonTesting/Backgrounds/Caesium" + (i + 1).ToString());
            }
            ...
            {
                Main.spriteBatch.Draw(value5, vector, value3, Color.White * AvalonTesting.CaesiumTransition, 0f, Vector2.Zero, num25, SpriteEffects.None, 0f);
                if (layerTextureIndex == 0)
                {
                    int num14 = (int)(vector.Y + value3.Height * num25);
                    Main.spriteBatch.Draw(TextureAssets.BlackTile.Value, new Rectangle((int)vector.X, num14, (int)(value3.Width * num25), Math.Max(0, Main.screenHeight - num14)), new Color(6, 5, 6) * AvalonTesting.CaesiumTransition);
                }
                vector.X += num18;
            }
        });
