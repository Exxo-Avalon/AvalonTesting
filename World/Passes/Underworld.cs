    ...
    public static void Method(GenerationProgress progress, GameConfiguration configuration)
    {
        ...
        for (int q = (Main.maxTilesX - Main.maxTilesX / 5) - 15; q < Main.maxTilesX - Main.maxTilesX / 5; q++)
        {
            for (int z = Main.maxTilesY - 250; z < Main.maxTilesY - 20; z++)
            {
                if (q > (Main.maxTilesX - Main.maxTilesX / 5) - 10)
                {
                    ...
                    {
                        WorldGen.TileRunner(q, z, WorldGen.genRand.Next(6, 8), WorldGen.genRand.Next(6, 8), ModContent.TileType<Tiles.BlastedStone>());
                    }
                }
            }
        }
        for (int q = Main.maxTilesX - Main.maxTilesX / 5; q < Main.maxTilesX - 20; q++)
        {
            for (int z = Main.maxTilesY - 250; z < Main.maxTilesY - 20; z++)
            {
                ...
                if ((Main.tile[q, z].TileType == TileID.Ash || Main.tile[q, z].TileType == TileID.Hellstone) && Main.tile[q, z].HasTile) Main.tile[q, z].TileType = (ushort)ModContent.TileType<Tiles.BlastedStone>();
            ...
            for (int z = Main.maxTilesY - 250; z < Main.maxTilesY - 20; z++)
            {
                if (q % 100 < 33 && z > Main.maxTilesY - 175)
                {
                    if (Main.tile[q, z].HasTile && !Main.tile[q, z - 1].HasTile ||
                        ...
                        Main.tile[q, z].HasTile && !Main.tile[q + 1, z].HasTile)
                    {
                        if (Main.tile[q, z].TileType == ModContent.TileType<Tiles.BlastedStone>())
