...
public class LockedContagionChest : ModTile
{
    public override void SetStaticDefaults()
    {
        ...
        name.SetDefault("Locked Contagion Chest");
        AddMapEntry(new Color(200, 20, 0), name, MapChestName);
        TileID.Sets.DisableSmartCursor[Type] = true;
        AdjTiles = new int[] { TileID.Containers };
        ContainerName.SetDefault("Locked Contagion Chest");
    ...
    public override void MouseOver(int i, int j)
    {
        ...
        {
            player.cursorItemIconText = Main.chest[chest].name.Length > 0 ? Main.chest[chest].name : "Locked Contagion Chest";
            if (player.cursorItemIconText == "Locked Contagion Chest")
            {
