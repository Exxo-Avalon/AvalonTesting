...
public class test5 : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        swingRadius = swingRadius.RotatedBy(0.6f * swordVel * player.direction * posY);
    ...
    {
        Texture2D texture = ModContent.Request<Texture2D>("Avalon/Projectiles/Melee/test5", AssetRequestMode.ImmediateLoad).Value;
        Rectangle frame = texture.Frame();
        Vector2 drawPos = Projectile.Center - Main.screenPosition;
        for (int i = 0; i < Projectile.oldPos.Length; i++)
        {
            Vector2 drawPosOld = Projectile.oldPos[i] - Main.screenPosition + Projectile.Size / 2f;
            Main.EntitySpriteDraw(texture, drawPosOld, frame, new Color(255, 255, 255, 225) * (1 - (i * 0.25f)) * 0.25f, Projectile.oldRot[i], frame.Size() / 2f + new Vector2(28f, -27f), Projectile.scale, SpriteEffects.None, 0);
        }
        Main.EntitySpriteDraw(texture, drawPos, frame, new Color(255, 255, 255, 225), Projectile.rotation, frame.Size() / 2f + new Vector2(28f, -27f), Projectile.scale, SpriteEffects.None, 0);
        return false;
    }
}
