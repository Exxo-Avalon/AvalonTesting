...
public class CaesiumSpike : ModProjectile
{
    ...
    {
        Texture2D texture = ModContent.Request<Texture2D>("Avalon/Projectiles/Hostile/CaesiumSpike").Value;
        Vector2 origin = new Vector2(texture.Width * 0.5f, Projectile.height * 0.5f);
        if (Projectile.spriteDirection == -1)
        {
            Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition, null, Color.White * ((255f - Projectile.alpha) / 255f), Projectile.rotation, origin, Projectile.scale, SpriteEffects.FlipHorizontally, 0);
        }
        else
        {
            Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition, null, Color.White * ((255f - Projectile.alpha) / 255f), Projectile.rotation, origin, Projectile.scale, SpriteEffects.None, 0);
        }
        counter++;
        if (counter > 22)
            counter = 0;
        for (int j = 0; j < 2; j++)
        {
            ...
            if (Projectile.spriteDirection == -1)
            {
                Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition + offset, null, new Color(100, 100, 100, 0) * bonusAlphaMult * ((255f - Projectile.alpha) / 255f), Projectile.rotation, origin, 1.00f, SpriteEffects.FlipHorizontally, 0);
            }
            else
            {
                Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition + offset, null, new Color(100, 100, 100, 0) * bonusAlphaMult * ((255f - Projectile.alpha) / 255f), Projectile.rotation, origin, 1.00f, SpriteEffects.None, 0);
            }
        }
    }
