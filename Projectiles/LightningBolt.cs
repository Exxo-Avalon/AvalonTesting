...
public class LightningBolt : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile.scale == 1.0)
        {
            ...
            {
                int num54 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position, Vector2.Zero, ModContent.ProjectileType<LightningTrail>(), 60, 0f, Projectile.owner);
        ...
        if (((Main.rand.Next(15) == 0 && Projectile.ai[0] > 0) || Projectile.scale == 1.0) && Projectile.scale > 0.4f)
        {
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                int lightning = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position, Vector2.Zero, ModContent.ProjectileType<LightningBolt>(), 80, 0f, Projectile.owner);
