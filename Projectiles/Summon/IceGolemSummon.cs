...
public class IceGolemSummon : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        var flag12 = false;
        var num356 = 85;
        Main.player[Projectile.owner].AddBuff(ModContent.BuffType<IceGolem>(), 3600);
        ...
        if (timer > 180)
        {
            int n = Projectile.FindClosestNPC(1600, (npc)=>npc.townNPC && npc.lifeMax <= 5 && npc.dontTakeDamage);
            if (n != -1)
            {
                ...
                else
                {
                    ...
                    else if (num421 >= 0 && num419 < 800f + num414)
                    {
                        ...
                        else if (Projectile.owner == Main.myPlayer)
                        {
                            ...
                            var num436 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), vector32.X, vector32.Y, num430, num432, num435, num434, (Projectile.type == ModContent.ProjectileType<IceGolemSummon>()) ? 8f : Projectile.knockBack, Main.myPlayer, 0f, 0f);
