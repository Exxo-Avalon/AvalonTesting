        ...
        if (Projectile.type == ModContent.ProjectileType<Minisphere>())
        {
            ...
            if (flag21)
            {
                ...
                if (Projectile.localAI[0] > 8f)
                {
                    ...
                    num594 *= num596;
                    num595 *= num596;
                    var num597 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), value7.X, value7.Y, num594, num595, ProjectileID.MagnetSphereBolt, (Projectile.type == ModContent.ProjectileType<Minisphere>()) ? (Projectile.damage / 3 * 2) : Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    if (Projectile.type == ModContent.ProjectileType<Minisphere>())
                    {
