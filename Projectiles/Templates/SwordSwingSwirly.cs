...
public abstract class SwordSwingSwirly : ModProjectile
{
    ...
    Vector2 SwingHerePlease;
    public override void OnSpawn(IEntitySource source)
    {
        SwingHerePlease = Main.player[Projectile.owner].Center.DirectionTo(Main.MouseWorld);
    }
    public override void AI()
    {
        ...
        Projectile.rotation = (float)Math.PI * 2f * num2 * num + num3 + num2 * (float)Math.PI + player.fullRotation;
        float num5 = 1f;
        float num6 = 1.2f;
        Projectile.velocity = Vector2.Lerp(Vector2.Zero,new Vector2(50,0).RotatedBy(SwingHerePlease.ToRotation()),num);
    ...
    public void DrawProj_Excalibur(Projectile proj)
    {
        ...
        float[] SwingSpread = { 0.3f, 0.6f, 0.9f, 1.2f, 1.2f };

        Main.spriteBatch.Draw(val, vector, rectangle, color1 * fromValue * num3, proj.rotation + (float)(Math.PI * SwingSpread[4] * player.direction), origin, num * scalemod, effects, 0f);
        Main.spriteBatch.Draw(val, vector, rectangle, color5 * 0.15f, proj.rotation + (float)(Math.PI * SwingSpread[3] * player.direction), origin, num * scalemod * 0.9f, effects, 0f);
        Main.spriteBatch.Draw(val, vector, rectangle, color3 * 0.4f, proj.rotation + (float)(Math.PI * SwingSpread[2] * player.direction), origin, num * scalemod * 0.8f, effects, 0f);
        Main.spriteBatch.Draw(val, vector, rectangle, Color.Lerp(color2,color3,0.5f) * 0.4f, proj.rotation + (float)(Math.PI * SwingSpread[1] * player.direction), origin, num * scalemod * 0.7f, effects, 0f);
        Main.spriteBatch.Draw(val, vector, rectangle, color2 * 0.4f, proj.rotation + (float)(Math.PI * SwingSpread[0] * player.direction), origin, num * num4 * scalemod * 0.6f, effects, 0f);
